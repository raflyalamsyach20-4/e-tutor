...
<head>
  ...
  <title>Reset Password — E-Tutor Premium</title>
    ...
    <div class="bg-white/90 backdrop-blur-xl border border-white rounded-[32px] p-10 sm:p-12 shadow-[0_20px_60px_-15px_rgba(15,76,129,0.15)] animate-fade-in-up">
      ...
      <div class="mb-10 text-center animate-fade-in-up stagger-1">
        <div class="inline-flex items-center justify-center w-20 h-20 bg-brand-blue border border-brand-blue/20 rounded-[24px] mb-6 shadow-[0_10px_25px_rgba(15,76,129,0.2)]">
          <span class="iconify text-brand-yellow text-4xl" data-icon="lucide:lock-keyhole"></span>
        </div>
        <h1 class="text-[32px] font-extrabold text-brand-blue tracking-tight mb-3">Buat Password Baru</h1>
        <p class="text-[15px] text-slate-500 font-medium leading-relaxed px-2">
          Keamanan akun Anda adalah prioritas kami. Silakan atur password baru untuk melanjutkan akses.
        </p>
      </div>
      ...
      @if(session('error'))
        <div class="animate-fade-in-up stagger-2 bg-brand-red/5 border border-brand-red/20 text-brand-red px-5 py-4 rounded-[16px] mb-8 flex items-start gap-3 shadow-sm">
          ...
          <p class="text-[13px] font-bold">{{ session('error') }}</p>
        </div>
      @endif
      ...
      <form action="{{ route('password.update') }}" method="POST" class="space-y-6">
        ...
        <div class="space-y-2 group animate-fade-in-up stagger-2">
          ...
          <div class="relative">
            ...
            <input type="email" id="email" name="email" value="{{ $email ?? old('email') }}" required readonly
                   class="w-full pl-12 pr-4 py-4 bg-slate-50/50 border border-slate-200 rounded-[16px] text-[14px] text-slate-500 font-bold focus:outline-none cursor-not-allowed">
          </div>
          @error('email') <p class="text-brand-red text-[11px] font-extrabold mt-2 flex items-center gap-1.5 uppercase tracking-widest"><span class="iconify text-sm" data-icon="lucide:alert-circle"></span> {{ $message }}</p> @enderror
        </div>

        <div class="space-y-2 group animate-fade-in-up stagger-2">
          <label class="block text-[12px] font-extrabold text-brand-blue uppercase tracking-widest" for="password">Password Baru</label>
          <div class="relative">
          ...
          @error('password') <p class="text-brand-red text-[11px] font-extrabold mt-2 flex items-center gap-1.5 uppercase tracking-widest"><span class="iconify text-sm" data-icon="lucide:alert-circle"></span> {{ $message }}</p> @enderror
        </div>

        <div class="space-y-2 group animate-fade-in-up stagger-2">
          <label class="block text-[12px] font-extrabold text-brand-blue uppercase tracking-widest" for="password_confirmation">Konfirmasi Password</label>
          <div class="relative">
        ...
        <div class="animate-fade-in-up stagger-3 pt-2">
            <button type="submit" class="btn-shine w-full py-4 bg-brand-yellow hover:bg-yellow-400 text-brand-blue text-[15px] font-extrabold rounded-[16px] shadow-[0_8px_20px_rgba(245,158,11,0.3)] hover:shadow-[0_12px_25px_rgba(245,158,11,0.4)] transform hover:-translate-y-1 transition-all focus:outline-none focus:ring-4 focus:ring-brand-yellow/30 flex items-center justify-center gap-2">
              Ubah Password <span class="iconify text-xl" data-icon="lucide:check-circle"></span>
            </button>
        </div>
      </form>
